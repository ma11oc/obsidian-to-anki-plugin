...
export class SettingsTab extends PluginSettingTab {
  setup_custom_regexp (note_type: string, row_cells: HTMLCollection) {
    ...
    const custom_regexp = new Setting(row_cells[1] as HTMLElement)
      .addText(
        text => text.setValue(
  ...
  setup_link_field (note_type: string, row_cells: HTMLCollection) {
    ...
    const link_field = new Setting(row_cells[2] as HTMLElement)
      .addDropdown(
        async dropdown => {
  ...
  setup_context_field (note_type: string, row_cells: HTMLCollection) {
    ...
    const context_field = new Setting(row_cells[3] as HTMLElement)
      .addDropdown(
        async dropdown => {
          const field_names = plugin.fields_dict[note_type]
          for (const field of field_names) {
  ...
  get_folders (): TFolder[] {
    ...
    for (const folder of folder_list) {
      const filtered_list: TFolder[] = folder.children.filter((element) => element.hasOwnProperty('children')) as TFolder[]
  ...
  setup_folder_deck (folder: TFolder, row_cells: HTMLCollection) {
    ...
    const folder_deck = new Setting(row_cells[1] as HTMLElement)
      .addText(
  ...
  setup_folder_tag (folder: TFolder, row_cells: HTMLCollection) {
    ...
    const folder_tag = new Setting(row_cells[2] as HTMLElement)
      .addText(
